...
def analyze_fidelity(root_dir: str = "neps_results"):
    ...
    for trial_path in sorted(configs_path.glob("config_*_*")):
        try:
            ...
            metadata_file = trial_path / "metadata.json"
    ...
    plt.xlabel(f'Loss at Low Fidelity (data_fraction={low_fidelity_val:.2f})')
    plt.ylabel(f'Loss at High Fidelity (data_fraction={high_fidelity_val:.2f})')
    ...
    plot_filename = "fidelity_correlation_analysis.png"
...
if __name__ == "__main__":
    ...
    analyze_fidelity(root_dir=args.path)